...
getBatchByProducts=(req,res)=>{
    ...
    if(Verifytoken(req.params.token,req.params.username)){
        let date=new Date();
        batch.findAll({where:{ProdID:req.params.id}}).then(
            data=>{
                var Batcharray=[];
        ...
        .catch(err=>{
            res.status(200).json({status:false,data:"Data not Found"});
        })
    }
    else{
...
deleteBatch=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        batch.destroy({where:{BatchID:req.params.id}}).then(response=>{
            res.status(200).json({message:true,data:"Batch Deleted"});
        })
        .catch(err=>{
            res.status(200).json({message:false,data:"Batch Not Deleted"});
        })
    }
    else{
        res.status(200).json({message:false,data:"Unauthenticated"});
