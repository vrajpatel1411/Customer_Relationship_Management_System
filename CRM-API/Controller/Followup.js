        ...
        .then(data=>{
            if(data.length>0){
                data.forEach(element => {
                    element.FollowUpDate=require('moment')(element.FollowUpDate).format('YYYY-MM-DD');
                });
...
const addFollowUp=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        followUp.create({ClientID:req.body.ClientID,LeadID:req.body.LeadId,FollowUpDate:req.body.FollowUpDate,ContactType:req.body.ContactType,Remarks:req.body.Remarks})
        .then(()=>{
...
const getSpecificFollowUp=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        followUp.findAll({where:{LeadID:req.params.id}})
        .then(data=>{
            if(data.length>0){
                data.forEach(element => {
                    element.FollowUpDate=require('moment')(element.FollowUpDate).format('YYYY-MM-DD');
...
const updateFollowUp=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        followUp.update(req.body,{where:{FollowUpID:req.params.id}})
        .then(()=>{
...
const deleteFollowup=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        followUp.destroy({where:{FollowUpID:req.params.id}})
        .then(()=>{
