        ...
        .then(data=>
            {
                if(data.length>0)
                {
                    data.forEach(element => {
                        element.StatusDate=require('moment')(element.StatusDate).format('YYYY-MM-DD');
                    });
...
getSpecificInterest=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        interest.findAll({where:{LeadID:req.query.leadid,Status:req.query.status}})
        .then(data=>{
            if(data.length>0)
            {
                data.forEach(element => {
...
updateInterest=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        interest.update(req.body,{where:{InterestedInID:req.params.id}})
        .then(()=>{
...
deleteInterest=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        interest.destroy({where:{InterestedInID:req.params.id}})
        .then(()=>{
...
deletedbyidandleadid=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        interest.destroy({where:[{ProdID:req.query.ProdID,LeadID:req.query.LeadID}]})
        .then(()=>{
...
UpdateInterestByProduct=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        interest.update(req.body,{where:{ProdID:req.query.prodid,LeadID:req.query.leadid}})
        .then(()=>{
            res.status(200).json({message:true,data:"Data Updated"});
        })
        .catch(err=>
...
LeadInterest=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        ...
        interest.findAll({where:{LeadID:leadid,ProdID:prodid}})
        .then(data=>{
            if(data.length>0)
            {
                data.forEach(element => {
                    element.InterestedInStatusDate=require('moment')(element.StatusDate).format('YYYY-MM-DD');
                });
                res.status(200).json({message:true,data:data});
            }
            else{
