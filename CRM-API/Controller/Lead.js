...
getLeadWithClientID=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        lead.findAll({where:{UserID:req.query.userid,Status:req.query.status}}).then(data=>{
            console.log(data);
            if(data.length>0)
            {
                data.forEach(element => {
                    element.LeadDate=require('moment')(element.LeadDate).format('YYYY-MM-DD');
                    element.LeadStatusDate=require('moment')(element.LeadStatusDate).format('YYYY-MM-DD');
                    element.NextFollowUpDate=require('moment')(element.NextFollowUpDate).format('YYYY-MM-DD');
...
addLead=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        ...
        .then((response)=>{
            res.status(200).json({message:true,data:"Added",leadid:response.dataValues.LeadID});
        })
        .catch((err)=>{
...
getSpecificLead=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        lead.findAll({where:{LeadID:req.params.id}})
        .then(data=>{
            if(data.length>0)
            {
                data.forEach(element => {
...
updateLead=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        ...
        { 
            ...
            lead.update(req.body,{where:{LeadID:req.params.id}})
                .then(()=>{
...
deleteLead=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        lead.destroy({where:{LeadID:req.params.id}})
        .then(()=>{
...
assignLead=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        lead.findAll({where:{UserID:null}}).then(data=>{
            var Leadarray=[];
            ...
            if(Leadarray.length>0){
                ...
                console.log(leadid);
                lead.update({UserID:req.params.id},{where:{LeadID:leadid}})
                .then(()=>{
                    res.status(200).json({message:true,data:"Data Updated"});
                })
                .catch(err=>
...
updateUserID=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        lead.update({UserID:req.body.UserID},{where:{UserID:req.params.id}})
        .then(response=>{
...
module.exports={
    ...
    getSpecificLead,
    updateLead,
    deleteLead,getLeadWithClientID,
    assignLead,
    updateUserID
}