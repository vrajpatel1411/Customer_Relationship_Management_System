...
getSpecificProduct=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        product.findAll({where:{ProdID:req.params.id}})
        .then(data=>{
            if(data.length>0)
            {
                res.status(200).json({message:true,data:data});
...
getProductbyClient=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        product.findAll({where:{ClientID:req.params.id}})
        .then(data=>{
            if(data.length>0)
            {
                res.status(200).json({message:true,data:data});
...
updateProduct=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
        ...
        product.update(req.body,{where:{ProdID:req.params.id}})
        .then(()=>{
...
deleteProduct=(req,res)=>{
    if(Verifytoken(req.params.token,req.params.username)){
     ...
     product.destroy({where:{ProdID:req.params.id}})
    .then(()=>{
